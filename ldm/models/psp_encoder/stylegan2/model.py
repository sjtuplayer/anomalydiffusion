...
class Discriminator2(nn.Module):
    ...
    def __init__(self, image_size=256, conv_dim=64,  repeat_num=6,linear=False):
        ...
        self.linear=linear
        if self.linear:
            self.linear_mapping=nn.Linear(4,1)
    def forward(self, x):
        pred = self.main(x).view(x.size(0),-1)
        if self.linear:
            pred=self.linear_mapping(pred)
        return pred

class SplitDiscriminator(nn.Module):
    def __init__(self, input_nc, aus_nc, image_size=256, ndf=64, n_layers=7, norm_layer=nn.BatchNorm2d):
        super(SplitDiscriminator, self).__init__()
        use_bias = norm_layer.func =True
        kw = 4
        padw = 1
        sequence = [
